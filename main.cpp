...
int main(int argc, char *argv[])
{
    ...
    MPI_Type_create_struct(5, lengths, displacements, types, &mpiDatatype);
    ...
    if (node == 0) {
        ...
        struct ToCalculate singleNodeToCalculate;
        singleNodeToCalculate.a = a;
        singleNodeToCalculate.b = b;
        singleNodeToCalculate.c = c;
        singleNodeToCalculate.start = start;
        singleNodeToCalculate.end = end;
        IntegralCalculator singleNodeCalculator(singleNodeToCalculate);
        t1 = MPI_Wtime();
        singleNodeCalculator.calculate_trapezoidal_rule();
        t1 = MPI_Wtime() - t1;
    ...
    if (node == 0)
    {
        for (int i = 0; i < Nnodes; i++) {
            ...
            if (i+1 != Nnodes) {
            MPI_Send(&toBeCalculated, 1, mpiDatatype, i+1, tag, MPI_COMM_WORLD);
            } else {
    ...
    else
    {
        ...
        MPI_Recv(&receivedCalculate, 1, mpiDatatype, 0, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    if (node == 0) {
        tp = MPI_Wtime() - tp;
        printf("Parallized calculation took: %f seconds\n", tp);
        printf("Single node, calculation took: %f seconds\n", t1);
        printf("Result: %f\n", result);
    }
